...
- API клиентите могат да заменят шаблона за конкретна фактура чрез `template_id` / `template_name`; ако не са зададени, се прилага съществуващата верига: шаблон на доставчика → глобален по подразбиране
...
- Опционалните `template_id` / `template_name` избират шаблона за конкретната фактура (`template_id` има приоритет; `template_name` е нечувствителен към главни/малки букви). Ако не са зададени, се прилага съществуващата верига по подразбиране: шаблон на доставчика → глобален по подразбиране. Непознато id или име връща **400 Bad Request** - няма мълчалив fallback. Същите полета работят и за асинхронния endpoint `POST /api/v1/invoice-requests`.
...
Тялото на заявката е идентично с `POST /api/v1/invoices/generate`, включително опционалното заменяне на шаблона чрез `template_id` / `template_name` - работникът на опашката прилага същата верига на разрешаване, когато рендерира. Полето `response_type` се игнорира - асинхронните заявки само записват реда в опашката; след завършване извлечете фактурата чрез стандартните endpoint-и за извличане, използвайки `invoiceNumber` от отговора на статуса.
