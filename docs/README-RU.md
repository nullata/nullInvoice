...
- API-клиенты могут переопределить шаблон для конкретного счёта через `template_id` / `template_name`; если ни одно поле не задано, действует существующая цепочка: шаблон по умолчанию поставщика → глобальный по умолчанию
...
- Опциональные `template_id` / `template_name` выбирают шаблон для этого счёта (`template_id` имеет приоритет; `template_name` регистронезависим). Если оба поля не заданы, действует существующая цепочка fallback: шаблон по умолчанию поставщика → глобальный по умолчанию. Неизвестный id или имя возвращают **400 Bad Request** - молчаливого fallback нет. Те же поля работают и на асинхронном endpoint `POST /api/v1/invoice-requests`.
...
Тело запроса идентично `POST /api/v1/invoices/generate`, включая опциональную замену шаблона через `template_id` / `template_name` - воркер очереди применяет ту же цепочку разрешения при рендеринге. Поле `response_type` игнорируется - асинхронные запросы только сохраняют строку в очереди; после завершения извлеките счёт через стандартные endpoint'ы с использованием `invoiceNumber`, возвращённого в ответе о статусе.
